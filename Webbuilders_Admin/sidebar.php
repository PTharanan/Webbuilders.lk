...
<aside class="w-64 sidebar bg-black text-white flex flex-col transition-colors duration-500" id="sidebar">
  <div class="p-6 text-2xl font-bold flex items-center justify-between">        
      <img src="<?php echo (strpos($_SERVER['PHP_SELF'], '/2checkout/') !== false ? '../' : './'); ?>assets/webbuildersLogo.png" alt="Logo" class="ml-2 bg-white h-22 w-auto"/>
  ...
  <nav class="flex-1 px-4 py-2 overflow-y-auto custom-scrollbar">
    <ul>
      ...
      <li class="my-2"><a href="interviews.php" class="block px-4 py-3 rounded-lg <?php echo basename($_SERVER['PHP_SELF']) == 'interviews.php' ? 'bg-white/10' : 'hover:bg-white/10'; ?> transition-all duration-300 flex items-center gap-3"><i class="fas fa-id-card w-5 text-center"></i> Interviews</a></li>
      <li class="my-2"><a href="adminDocs.php" class="block px-4 py-3 rounded-lg <?php echo basename($_SERVER['PHP_SELF']) == 'adminDocs.php' ? 'bg-white/10' : 'hover:bg-white/10'; ?> transition-all duration-300 flex items-center gap-3"><i class="fas fa-id-card w-5 text-center"></i> Employees</a></li>
  ...
  <div class="p-6 border-t border-white/20">
    <div class="flex items-center space-x-3 mb-4">
      ...
      <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=100&q=80" 
           alt="User" 
           class="rounded-full border-2 border-white w-10 h-10 object-cover"
    ...
    <button id="logout-btn" class="w-full mt-4 px-4 py-3 rounded-lg bg-white/10 hover:bg-white/20 transition-all duration-300 flex items-center gap-3 text-white">
...
<div id="logout-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 hidden">
  <div class="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full mx-4 transform transition-all duration-300 scale-95 opacity-0" id="logout-modal-content">
    <div class="text-center">
      ...
      <p class="text-gray-600 dark:text-gray-300 mb-6">Are you sure you want to logout? You will need to sign in again.</p>
      <div class="flex gap-3">
        <button id="cancel-logout" class="flex-1 px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
          Cancel
        </button>
