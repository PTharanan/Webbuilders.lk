...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($email) || empty($password)) {
    ...
    try {
      ...
      $stmt = $conn->prepare("SELECT id, email, password, name, role, status FROM users WHERE email = ?");
      $stmt->execute([$email]);
...
<body class="font-sans antialiased bg-surface text-navy overflow-x-hidden">
  ...
  <main class="min-h-screen flex items-center justify-center px-4 sm:px-6">
    ...
    <!-- Floating Elements -->
    <div
      class="absolute top-1/4 left-1/4 w-6 h-6 bg-tc rounded-full opacity-70 premium-float-animation hidden sm:block">
    </div>
    <div
      class="absolute top-1/3 right-1/4 w-4 h-4 bg-premium-gold rounded-full opacity-60 premium-float-animation hidden sm:block"
      style="animation-delay: 1s;"></div>
    <div
      class="absolute bottom-1/4 left-1/3 w-8 h-8 bg-tc-600 rounded-full opacity-40 premium-float-animation hidden sm:block"
  ...
  <script>
    ...
    function showToast(message, type = 'success') {
      ...
      toast.className = `flex items-center p-4 rounded-lg shadow-lg transform translate-x-full transition-all duration-300 max-w-sm ${type === 'success' ? 'bg-green-500 text-white' : 'bg-red-500 text-white'
        }`;
